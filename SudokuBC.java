...
public class Sudoku
{
	...
	private boolean[][] changedSudoku = new boolean[9][9];
	...
	public static void main(String[] args)
	{
		...
		while(!sudoku.checkSolved() && stop < stopvalue)
		{
			...
			{
				int[] coords = sudoku.numberPossSudoku();
	...
	public boolean updatePossSudoku()
	{
		for(int i = 0; i < SUDOKU_LENGTH; i++)
		{
			for(int j = 0; j < SUDOKU_LENGTH; j++)
	...
	public int[] numberPossSudoku()
	{
		...
		for(int i = 0; i < 9; i++)
		{
			for(int j = 0; j < 9; j++)
			{
				int amountNotZero = countAmountNotZero(possSudoku[i][j]);
				
				if(amountNotZero > 1 && amountNotZero < coords[2])
				{
					coords[2] = amountNotZero;
					coords[0] = i;
					coords[1] = j;
				}
